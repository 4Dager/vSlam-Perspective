...
bool FeatureTracker::solveVinsPnP(double header, Eigen::Vector3d &P, Matrix3d &R, bool vins_normal)
{
    if(!vins_normal)//if it is in initial time, return false.
        return false;
    
 
 //-----------reopen by cdx on 3.7-------------
  ...
  //------------------------------------------
    ...
    int i = 0;
    for (auto &it : solved_features)//solved_features get from non-liner part,then use lock to pass it to featuretracker.solved_features in .mm file.
    {
        while(ids[i] < it.id)
        {
            i++;
    ...
    for(auto &it : imu_msgs)//for each imu data do processIMU like the same in mono.
    {
        double t = it.header;
        if (current_time < 0)
            current_time = t;
    ...
    printf("image %lf\n", header);
    vins_pnp.processImage(feature_msg, header);// for each image data do processimage like the same in mono.
    
    P = vins_pnp.Ps[PNP_SIZE - 1];
    R = vins_pnp.Rs[PNP_SIZE - 1];

    Vector3d R_ypr = Utility::R2ypr(R);
    printf("debug  pnp P: %lf %lf %lf R: %lf %lf %lf %lu\n",P.x(), P.y(), P.z(), R_ypr.x(), R_ypr.y(), R_ypr.z(), feature_msg.size());

    return true;
}

void FeatureTracker::readImage(const cv::Mat &_img, cv::Mat &result, int _frame_cnt, vector<cv::Point2f> &good_pts, vector<double> &track_len, double header, Eigen::Vector3d &P, Matrix3d &R, bool vins_normal)
{
