...
/*
 assume pixel error is 1.5 pixel plane,sqrt_info is the sqrt of the information matrix, information matrix is the inverse of
 the covariance matrix. they set 1.5 as the visual measurement noise in the pixel plane.
 */
...
void VINS::new2old()
{
    ...
    if(ENABLE_LOOP_CLOSURE && loop_enable)
    {
        loop_enable = false;
        for(int i = 0; i< WINDOW_SIZE; i++)
        {
            ...
            {
                Matrix3d Rs_loop = Eigen::Quaterniond(front_pose.loop_pose[6],  front_pose.loop_pose[3],  front_pose.loop_pose[4],  front_pose.loop_pose[5]).normalized().toRotationMatrix();
...
/*
 ...
 Matrix3d rot_diff = Utility::ypr2R(Eigen::Vector3d(y_diff, 0, 0));
...
void VINS::processIMU(double dt, const Eigen::Vector3d &linear_acceleration, const Eigen::Vector3d &angular_velocity)
{
    ...
    if (frame_count != 0)
    {
        ...
        if(solver_flag != NON_LINEAR && tmp_pre_integration != nullptr) //comments because of recovering
...
void VINS::processImage(map<int, Eigen::Vector3d> &image_msg, double header, int buf_num)
{
    ...
    if(solver_flag == INITIAL)
    {
        ...
        tmp_pre_integration = std::make_shared<IntegrationBase>(acc_0, gyr_0, Eigen::Vector3d(0,0,0), Eigen::Vector3d(0,0,0));
        ...
        // 线程问题，数据可能进行了再次释放了。
        // 该函数中有 clear() 操作，务必保证 clear() 操作是原子的。
...
bool VINS::visualInitialAlign()
{
    ...
    Matrix3d yaw_refine = Utility::ypr2R(Eigen::Vector3d{-yaw0,0,0});
    R0 = yaw_refine * R0;
    g = R0 * g;
...
bool VINS::relativePose(int camera_id, Matrix3d &relative_R, Eigen::Vector3d &relative_T, int &l)
{
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
