...
public:
    virtual void operator()(const cv::Mat &im, const std::vector<cv::Point2f> window_pts,
                            std::vector<cv::KeyPoint> &keys, std::vector<DVision::BRIEF::bitset> &descriptors) const;
...
public:
    ...
    void rejectWithF(std::vector<cv::Point2f> &measurements_old,
    ...
    std::list<std::pair<int, Eigen::Matrix<double, 8, 1 > > > connection_list;
...
private:
    ...
    std::vector<DVision::BRIEF::bitset> window_descriptors;
