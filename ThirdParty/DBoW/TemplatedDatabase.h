...
{
    if(entry_id > m_dBowfile.size() - 1) {return;}
    printf("[my_debug]: m_dBowfile.size = %lu, entry_id = %d.\n", m_dBowfile.size(), entry_id);
...
//    assert(m_nentries == (int)fn.size());
      if(m_nentries != (int)fn.size()) {
          m_nentries = (int)fn.size();
      }
